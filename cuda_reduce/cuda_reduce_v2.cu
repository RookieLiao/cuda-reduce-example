...
int performCudaReductionV2() {
  ...
  reduce2<<<grid0, block>>>(idata_d, odata_d0, elem_size);
  reduce2<<<grid1, block>>>(odata_d0, odata_d1, grid0.x);
  reduce2<<<grid2, block>>>(odata_d1, odata_d2, grid1.x);
  ...
  printf("reduction_v2 elapsed %lf ms, bandwidth %lf GB/s", iElaps * 1e3, gpu_bw);
