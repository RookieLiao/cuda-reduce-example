...
__global__ void reduce7(int* idata_d, int* odata_d, size_t size) {
  ...
  // warp shuffle reduce
  if (tid < 32) {
    sdata[0] = warpReduce(sdata[tid]);
    sdata[32] = warpReduce(sdata[tid + 32]);
  }

  if (tid == 0) { odata_d[blockIdx.x] = sdata[0] + sdata[32]; }
}

int performCudaReductionV7(const size_t elem_size) {
  // set up device
  int dev = 0;
  cudaDeviceProp deviceProp;
  cudaGetDeviceProperties(&deviceProp, 0);
  printf("starting reduction at cuda_v7 device %d: %s\n", dev, deviceProp.name);
  ...
  reduce7<block_size><<<grid0, block, smem_size>>>(idata_d, odata_d0, elem_size);
  ...
  printf("reduction_v7 elapsed %lf ms, bandwidth %lf GB/s\n", iElaps * 1e3, gpu_bw);
